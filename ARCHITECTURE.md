...
```
cli/        入出力・ユーザー操作（端末依存）
  └─ index.ts / container.ts / commands/* / input.ts / format.ts
       │ 依存
core/usecases/   アプリのロジック（純粋・端末非依存）
  └─ chat.ts / analyze.ts / history.ts
       │ ポート（interface）越しに依存
core/ports/      抽象（契約だけ。実装を持たない）
  └─ ai-provider.ts (ProviderAgent) / storage.ts (Storage)
       ▲ implements
adapters/        外界とのつなぎ込み
  └─ ai/claude.ts ・ ai/gemini.ts ・ storage/yaml.ts ・ storage/memory.ts
```
